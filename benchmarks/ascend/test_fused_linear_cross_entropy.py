...
def test_correctness(
    ...
    num_elements_to_assign = torch.randint(1, B * T // 2,
                                           (1, )).item()  # Random number of elements to set to ignore_index
    ...
    def bench_fn(op, provider):
        ...
        if 'triton' in provider:
            ms = triton.testing.do_bench(lambda: liger_lm_head_ce(_input2, target), warmup=warmup, rep=rep)
        if 'pytorch' in provider:
            ms = triton.testing.do_bench(lambda: torch_lm_head_ce(_input1, target), warmup=warmup, rep=rep)
        return ms
...
def test_correctness_functional(B, T, H, V, scalar, dtype, bias, ce_weight, atol, rtol):
    ...
    y2, z2 = LigerFusedLinearCrossEntropyFunction.apply(x2, weight, target, bias, ce_weight, -100, 1e-4, 0.1, 'mean',
                                                        30.0, True)
