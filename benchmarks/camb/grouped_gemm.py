...
def main():
    ...
    b = torch.randn(z, n, k, dtype=torch.float16, device=device) if trans_b else torch.randn(
        z, k, n, dtype=torch.float16, device=device)
    ...
    def bench_fn(z, op, provider, device=device_):
        ...
        if 'triton' in provider:
            if 'fwd' == op:
                fn = lambda: GroupedGemm.apply(a, b, batch_sizes, trans_b)
            elif 'bwd' == op:
                c_triton = GroupedGemm.apply(a, b, batch_sizes, trans_b)
