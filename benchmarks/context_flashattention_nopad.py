...
def test():
    ...
    dtype = torch.float16
    Z = 1
    q = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype, device='cpu').normal_(mean=0.3, std=0.2)
    q_rope = torch.empty((Z * N_CTX, H, ROPE_HEAD), dtype=dtype, device='cpu').normal_(mean=0.3, std=0.2)

    kv = torch.empty((Z * N_CTX, 1, D_HEAD), dtype=dtype, device='cpu').normal_(mean=0.3, std=0.2)
    kv_rope = torch.empty((Z * N_CTX, 1, ROPE_HEAD), dtype=dtype, device='cpu').normal_(mean=0.3, std=0.2)

    o = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype, device='cpu').normal_(mean=0.7, std=0.2)
    o1 = torch.empty((Z * N_CTX, H, D_HEAD), dtype=dtype, device='cpu').normal_(mean=0.7, std=0.2)

    req_to_token_indexs = torch.zeros((10, Z * N_CTX), dtype=torch.int32, device='cpu')
    max_input_len = N_CTX
    Z = 1
    b_start_loc = torch.zeros((Z, ), dtype=torch.int32, device='cpu')
    b_seq_len = torch.ones((Z, ), dtype=torch.int32, device='cpu')
    b_req_idx = torch.ones((Z, ), dtype=torch.int32, device='cpu')
    b_prompt_cache_len = torch.zeros(1, dtype=torch.int32, device='cpu')
