...
def _gt(
    ...
    vllm._custom_ops.reshape_and_cache_new(
        k_states, v_states, k_caches, v_caches, kv_indices, "auto", 1.0, 1.0
    )
...
def test():
    ...
    k_caches = torch.full(
        (130, num_heads, head_dim//x, block_size, x), 0.0
    ).cuda()
    v_caches = torch.rand_like(k_caches).reshape(
        (130, block_size, num_heads, head_dim)
    )
