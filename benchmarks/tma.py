...
def add_kernel_tma(
    ...
    a = tl._experimental_descriptor_load(a_desc_ptr, [offs_m, offs_n], [BLOCK_SIZE_M, BLOCK_SIZE_N], tl.float32)
    b = tl._experimental_descriptor_load(b_desc_ptr, [offs_m, offs_n], [BLOCK_SIZE_M, BLOCK_SIZE_N], tl.float32)
...
def test_experimental_tma_matmul(num_stages, BLOCK_M, BLOCK_N, BLOCK_K):
    ...
    kernel = matmul_kernel_tl[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
        A,
        B,
        C,
        M,
...
def _test_tma():
    ...
    kernel = add_kernel_tma[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](desc_a,
                                                                                       desc_b,
                                                                                       desc_c,
                                                                                       M,
                                                                                       N,
    ...
    def bench_fn(op, provider, device=device):
        ...
        rep = 200
        if 'tma' in provider:
            fn = lambda: add_kernel_tma[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
                desc_a, desc_b, desc_c, M, N, BLOCK_M, BLOCK_N, num_warps=8, num_stages=1)
        if 'tl' in provider:
            fn = lambda: add_kernel_tl[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
                A_clone,
                B_clone,
                C_clone,
                M,
    ...
    x = torch.randn(SIZE, dtype=torch.float32, device=device)
    desc = np.empty(SIZE, dtype=np.int8)
    triton.runtime.driver.active.utils.fill_1d_tma_descriptor(x.data_ptr(), SIZE, BLOCK_SIZE, x.element_size(), desc)
    desc = torch.tensor(desc, device=device)
    z_tri = torch.empty_like(x)
    ...
    def bench_fn(op, provider, device=device):
        ...
        if 'tma' in provider:
            if 'copy' == op:
                fn = lambda: load_kernel_tma[(1, )](z_tri, desc, SIZE, BLOCK_SIZE, num_warps=1)
            elif 'matmul' == op:
                fn = lambda: matmul_kernel_tma[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
                    desc_a,
                    desc_b,
                    desc_c,
                    M,
        ...
        if 'tl' in provider:
            if 'copy' == op:
                fn = lambda: load_kernel_tl[(1, )](tl_res, x, SIZE, BLOCK_SIZE, num_warps=1)
            elif 'matmul' == op:
                fn = lambda: matmul_kernel_tl[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
                    A_tl,
                    B_tl,
                    C_tl,
                    M,
