...
def fused_topkgating(logits: Tensor,
                     k: int,
                     capacity_factor: float = 1.0,
                     min_capacity: int = 2) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
...
def fused_topkgating_opt(
    ...
    expert_sel_top_c_token_idx = torch.topk(torch.sum(token_sel_exp_int_mask, dim=0), k=capacity, dim=0, sorted=True)[1]
...
def test():
    ...
    output1_torch, out_torch_pack = model_torch(logits_torch, k, capacity_factor, min_capacity,
    ...
    output1_triton, output2_triton, output3_triton, output4_triton = model_triton(logits_triton, k, capacity_factor,
                                                                                  min_capacity, False)
    ...
    if False:
        with profile(
            ...
            output1_triton, output2_triton, output3_triton, output4_triton = (model_triton(
                logits_triton, k, capacity_factor, min_capacity, True))
            loss_torch = torch.sum(torch.mean(output1_torch * output3_torch))
            loss_triton = torch.sum(torch.mean(output1_triton * output3_triton))
            dout_torch = torch.randn_like(loss_torch)
            with torch.no_grad():
    ...
    def bench_top2gating(SeqLen, op, provider, device=device_):
        ...
        if 'triton' in provider:
            if 'fwd' == op:
                ms = triton.testing.do_bench(
                    lambda: model_triton(logits, k, capacity_factor, min_capacity, enable_token_rearrange_opt),
                    warmup=warmup,
                    rep=rep)
            elif 'bwd' == op:
                ...
                ms = triton.testing.do_bench(lambda: loss.backward(retain_graph=True), warmup=warmup, rep=rep)
            else:
                raise Exception()
        ...
        if 'pytorch' in provider:
            if 'fwd' == op:
                ms = triton.testing.do_bench(
                    lambda: model_torch(logits, k, capacity_factor, min_capacity, enable_token_rearrange_opt),
                    warmup=warmup,
                    rep=rep)
            elif 'bwd' == op:
                # def iter(logits, k, capacity_factor, min_capacity, enable_token_rearrange_opt):
