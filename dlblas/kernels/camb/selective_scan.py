...
def call(u, delta, A, B, C, D, initial_state=None):
    # D = D.float()
    # A = A.float()
...
for dtype in [torch.float32]:
    for device in ['cuda']:
        B, T = SymVar('B'), SymVar('T')
        D, K = SymVar('D'), SymVar('K')
        # we dont' actually allocate tensor
        AA = Tensor((D, K), dtype=dtype, device=device)
        xx = Tensor((B, T, D), dtype=dtype, device=device)
