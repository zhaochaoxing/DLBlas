...
from dlblas.kernels.camb import topk_gating_fwd_part1, topk_gating_fwd_part2, topk_gating_fwd_part3, topk_gating_bwd
...
class TopKGatingFunc(torch.autograd.Function):
    ...
    def forward(ctx: torch.Any, logits: torch.Tensor, k: int, capacity_factor: float = 1.0, min_capacity: int = 2, higher_precision: bool = False):
        ...
        gates, masks = topk_gating_fwd_part1.call(logits, k)
        locations, res, ce = topk_gating_fwd_part2.call(gates, masks, k)
        ...
        if higher_precision:
            ...
            part3_res = topk_gating_fwd_part3.call(gates, masks, locations, k, capacity, True)
            l_aux = torch.mean(res)
    ...
    def backward(ctx: torch.Any, *grad_outputs: torch.Any) -> torch.Any:
        if len(ctx.saved_tensors) == 4:
            ...
            grad_logits = topk_gating_bwd.call(grad_l_aux, locations, masks, gates, ce)
