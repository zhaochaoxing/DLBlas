...
def permute_topK(input_act: torch.Tensor,
              indices: torch.Tensor,
              num_out_tokens: int,
              max_token_num: int):
    workspace_fw = []
    pdtype = torch.float32
    if pdtype != input_act.dtype:
      pdtype = input_act.dtype
      workspace_fw = []

    max_expanded_token_num = 0
    num_topK = indices.size(1)
    input_max_expanded_token_num = max(max_token_num, input_act.size(0)) * num_topK
    if max_expanded_token_num < input_max_expanded_token_num:
      max_expanded_token_num = input_max_expanded_token_num
      workspace_fw = []
...
def permute_topK_test(
    ...
    max_token_num = 0
    num_out_tokens = 0
    new_permute_output, row_id_map = permute_topK(new_permute_input, indices, num_out_tokens, max_token_num)
