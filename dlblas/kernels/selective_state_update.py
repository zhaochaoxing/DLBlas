...
def call(state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):
    """
    Argument:
...
def bench_fn(state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):
    fn = lambda: call(state, x, dt, A, B, C, D, z, dt_bias, dt_softplus)
...
for dtype in [torch.float32, torch.float16, torch.bfloat16]:
    for device in ['cuda']:
        ...
        # with heads
        state = Tensor((batch, nheads, dim, dstate), dtype=dtype, device=device)
        x = Tensor((batch, nheads, dim), dtype=dtype, device=device)
        dt = Tensor((batch, nheads, dim), dtype=dtype, device=device)
        A = Tensor((nheads, dim, dstate), dtype=torch.float32, device=device)
        B = Tensor((batch, ngroups, dstate), dtype=torch.float32, device=device)
        C = Tensor((batch, ngroups, dstate), dtype=torch.float32, device=device)
        D = Tensor((nheads, dim), dtype=torch.float32, device=device)
        z = Tensor((batch, nheads, dim), dtype=dtype, device=device)
        dt_bias = Tensor((nheads, dim), dtype=torch.float32, device=device)
        register_dlblas_op(name, None, (state, x, dt, A, B, C, D, z, dt_bias, torch.SymBool), call, bench_fn, call)
        # without heads
        state = Tensor((batch, dim, dstate), dtype=dtype, device=device)
        x = Tensor((batch, dim), dtype=dtype, device=device)
        dt = Tensor((batch, dim), dtype=dtype, device=device)
