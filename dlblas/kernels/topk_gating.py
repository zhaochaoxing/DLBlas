...
from dlblas.kernels.topk_gating_bwd import call as topk_gating_bwd
from dlblas.kernels.topk_gating_fwd_part1 import call as topk_gating_fwd_part1
from dlblas.kernels.topk_gating_fwd_part2 import call as topk_gating_fwd_part2
from dlblas.kernels.topk_gating_fwd_part3 import call as topk_gating_fwd_part3
from dlblas.utils import SymVar
from dlblas.utils import Tensor as FakeTensor
from dlblas.utils import register_dlblas_op
...
@triton.autotune(
    configs=[triton.Config({'BLOCK_S': BS}, num_stages=s, num_warps=w) for BS in [8, 16] for s in [1] for w in [1]],
    key=['k', 's', 'e'],
)
@triton.jit
def _topk_gating_bwd_kernel_0(gates_ks, denom_s, clamp_denom_s, grad_token_exp_weights_ks, add_1_ks, stride_ks_k,
                              min_value: tl.constexpr, k: tl.constexpr, s: tl.constexpr, e: tl.constexpr,
    ...
    grad_token_exp_weights_ks_data = tl.load(grad_token_exp_weights_ks_ptrs,
                                             mask=offs_k[:, None] < k and offs_s[None, :] < s)
...
@triton.autotune(
    configs=[triton.Config({'BLOCK_S': BS}, num_stages=s, num_warps=w) for BS in [2, 4, 8] for s in [1] for w in [1]],
    key=['k', 's', 'e'],
)
@triton.jit
def _topk_gating_bwd_kernel_1(ce, masks_kse, grad_l_aux, scatter_kse, gates_se, gates_se_grad, stride_se_s,
                              stride_kse_k, stride_kse_s, k: tl.constexpr, s: tl.constexpr, e: tl.constexpr,
                              BLOCK_K: tl.constexpr, BLOCK_S: tl.constexpr):
    pid = tl.program_id(axis=0)
    offs_k = tl.arange(0, BLOCK_K)
    offs_s = pid * BLOCK_S + tl.arange(0, BLOCK_S)
    ...
    masks_kse_ptrs = masks_kse + offs_k[:, None, None] * stride_kse_k + offs_s[None, :, None] * stride_kse_s + offs_e[
        None, None, :]
    masks_kse_data = tl.load(masks_kse_ptrs, mask=offs_k[:, None, None] < k and offs_s[None, :, None] < s)
    scatter_kse_ptrs = scatter_kse + offs_k[:, None, None] * stride_kse_k + offs_s[
        None, :, None] * stride_kse_s + offs_e[None, None, :]
...
def topk_bwd_triton(gates_se, ce, masks_kse, gates_ks, indices_ks, denom_s, clamp_denom_s, grad_l_aux,
    ...
    zeros_1 = torch.ops.aten.zeros.default([k, s, e],
                                           dtype=gates_se.dtype,
                                           layout=torch.strided,
                                           device=gates_se.device)
...
class TopKGatingFunc(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        gates, masks = topk_gating_fwd_part1(logits, k)
        locations, res, ce = topk_gating_fwd_part2(gates, masks, k)
        l_aux = torch.mean(res)
        if higher_precision:
            token_rearranged_ec_idx, token_exp_weights, expert_select_token_idx, for_bwd = topk_fwd_triton(
                gates, masks, locations, capacity)
            ...
            part3_res = topk_gating_fwd_part3(gates, masks, locations, k, capacity, True)
            l_aux = torch.mean(res)
    ...
    def backward(ctx, *grad_outputs):
        if len(ctx.saved_tensors) == 4:
            ...
            grad_logits = topk_gating_bwd(grad_l_aux, locations, masks, gates, ce)
            return grad_logits, None, None, None, None
        else:
            grad_l_aux = grad_outputs[0]
            grad_token_exp_weights = grad_outputs[1]
            getes, ce, masks, gate_ks, indices_s, denom_s, clamp_denom_s = ctx.saved_tensors
            res = topk_bwd_triton(getes, ce, masks, gate_ks, indices_s, denom_s, clamp_denom_s, grad_l_aux,
                                  grad_token_exp_weights)
...
def bench_fn(logits: torch.Tensor,
             ...
             min_capacity: int = 2,
             enable_token_rearrange_opt: bool = False):
    ms = triton.testing.do_bench(lambda: call(logits, k, capacity_factor, min_capacity, enable_token_rearrange_opt),
                                 warmup=100,
                                 rep=100)
    return ms
...
for dtype in [torch.float16, torch.float32]:
    for device in ['cuda']:
        ...
        logits = FakeTensor((seqLen, experts), dtype=dtype, device=device)
        # space = ChoiceSpace([])
        register_dlblas_op(name, None, (logits, torch.SymInt, torch.SymFloat, torch.SymInt, torch.SymBool), call,
                           bench_fn, call)
