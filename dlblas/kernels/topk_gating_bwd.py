...
@triton.autotune(
    configs=[triton.Config({'BLOCK_S': BS}, num_stages=s, num_warps=w) for BS in [1, 2, 4] for s in [1] for w in [1]],
    key=['s'],
)
@triton.jit
def _topk_gating_bwd_kernel(locations_kse, ce, masks, gates, diag_mask, grad_logits, stride_kse_k, stride_kse_s,
                            grad_l_aux, min_value: tl.constexpr, k: tl.constexpr, s: tl.constexpr, e: tl.constexpr,
                            c: tl.constexpr, BLOCK_K: tl.constexpr, BLOCK_S: tl.constexpr):
...
def call(grad_l_aux, locations, masks, gates, ce):
    ...
    with torch.cuda.device(gates.device):
        _topk_gating_bwd_kernel[lambda META: (triton.cdiv(s, META['BLOCK_S']), )](locations,
                                                                                  ce,
                                                                                  masks,
                                                                                  gates,
                                                                                  diag_mask,
...
def bench_fn(grad_l_aux, locations, masks, gates, ce):
    ms = triton.testing.do_bench(lambda: call(grad_l_aux, locations, masks, gates, ce), warmup=20, rep=20)
    return ms
...
for dtype in [torch.float16, torch.float32]:
    for device in ['cuda']:
        ...
        register_dlblas_op(name, None, (grad_l_aux, locations, masks, gates, ce), call, bench_fn,
