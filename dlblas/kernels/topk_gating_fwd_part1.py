...
def _topk_gating_kernel_part1(
    ...
    for idx in tlc.static_range(K):
        all_ids = tl.broadcast_to(tl.arange(0, EXPERTS)[None,:], (BLOCK_S, EXPERTS))
        indices_with_idx = tl.broadcast_to(tl.expand_dims(tl.sum((tl.broadcast_to(tl.arange(0, K)[None, :], (BLOCK_S, K)) == idx) * indices_data, axis = 1), axis = 1), (BLOCK_S, EXPERTS))
        values_with_idx  = tl.broadcast_to(tl.expand_dims(tl.sum((tl.broadcast_to(tl.arange(0, K)[None, :], (BLOCK_S, K)) == idx) * values_data, axis = 1), axis = 1), (BLOCK_S, EXPERTS))
