...
def call(gates: torch.Tensor, masks: torch.Tensor, k: int, capacity: int, moe_aux_loss_coeff: float):
    s, e = gates.shape
...
def bench_fn(gates: torch.Tensor, masks: torch.Tensor, k: int, capacity: int, moe_aux_loss_coeff: float):
    fn = lambda: call(gates, masks, k, capacity, moe_aux_loss_coeff)
    ms = triton.testing.do_bench(fn, warmup=20, rep=20)
    return ms


# register
name = '_topk_gating_fwd_part2_position'
