...
def batch_test_models(config, checkpoint_file="checkpoint_throughput.json"):
    ...
    for model_info in model_paths:
        ...
        try:
            subprocess.run(["python", "test_throughput_once.py", model_name, common_prefix, "--backend", "pytorch", "--tp", "1", "--concurrency", "8", "--cache-max-entry-count", "0.4", "--prompt-tokens", "128", "--completion-tokens", "128", "--test-round", "3", "--warmup-round", "1"])
