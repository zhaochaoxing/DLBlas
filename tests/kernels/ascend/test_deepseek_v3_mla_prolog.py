...
hidden_states = torch.randn((b, seq_len, hidden_size), dtype=dtype_, device=device_)
...
matmul_qc_qr_out_same = torch.randn(
    [b, seq_len, num_heads * q_head_dim], dtype=dtype_, device=device_
)

kv_same = torch.randn(
    [b, seq_len, (kv_lora_rank + qk_rope_head_dim)], dtype=dtype_, device=device_
)


def mla_prolog_triton(out_matmul_cq):
...
def mla_prolog_ref(out_matmul_cq):
    ...
    # 减少累计误差
    q = matmul_qc_qr_out_same.view(b, seq_len, num_heads, q_head_dim)
    ...
    q_nope_out = (
        ...
        .contiguous()
        .view(b, seq_len, num_heads, kv_lora_rank)
    )
    # other branch
    matmul_ckv_kr_out = torch.matmul(hidden_states, weightDkvKr)
    # 减少累计误差
    kv_same_ = kv_same.view(b, seq_len, 1, kv_lora_rank + qk_rope_head_dim)
