# https://github.com/InternLM/lmdeploy/blob/v0.6.1/tests/pytorch/kernel/test_apply_rotary.py
import pytest
import torch
...
class TestApplyRotary:
    ...
    def q_states(self, seq_length, num_heads_q, feature_dim, dtype):
        yield torch.rand(
            seq_length.sum(), num_heads_q, feature_dim, dtype=dtype, device="cuda"
        )

    @pytest.fixture
    def k_states(self, seq_length, num_heads_k, feature_dim, dtype):
        yield torch.rand(
            seq_length.sum(), num_heads_k, feature_dim, dtype=dtype, device="cuda"
        )

    @pytest.fixture
    def position_ids_1d(self, seq_length, max_seqlen):
        yield torch.randint(
            0, max_seqlen.item(), (seq_length.sum().item(),), device="cuda"
        )
    ...
    @pytest.mark.parametrize(
        "dtype", [torch.bfloat16, torch.float16, torch.float32], indirect=True
    )
    @pytest.mark.parametrize(
        ("num_heads_q", "num_heads_k"), [(8, 8), (8, 4)], indirect=True
    )
